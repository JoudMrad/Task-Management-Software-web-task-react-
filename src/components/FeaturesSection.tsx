      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
          {features.map((feature, index) => (
            <div
              key={feature.title}
              className="group relative p-8 rounded-2xl glass-card cursor-pointer transition-all duration-300 ease-out hover:border-primary/40 hover:shadow-xl hover:-translate-y-2 hover:bg-gradient-to-b hover:from-card hover:to-primary/5 animate-fade-in"
              style={{ animationDelay: `${index * 0.08}s` }}
            >
              <div className="w-14 h-14 rounded-xl gradient-bg flex items-center justify-center mb-6 transition-all duration-300 group-hover:shadow-glow group-hover:scale-110 group-hover:rotate-3">
                <feature.icon className="w-7 h-7 text-primary-foreground transition-transform duration-300 group-hover:scale-110" />
              </div>
